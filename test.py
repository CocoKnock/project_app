...
# ---- DATA COLLECTION 1 (Camera) - COMMENTED OUT (image collection finished) ----
...
#     frame.grid_columnconfigure(0, weight=1)
#
#     make_label(frame, "Step 2: Camera Capture", font=FONT_SUB).grid(row=0, pady=10)
#     
...
#             stop_camera()
#             # After capturing the image, proceed to audio recording (step 3)
...
def load_data_collection_page_2():
    ...
    def set_label_next(label):
        ...
        # Skip camera capture (already collected images), go directly to audio recording
        switch_page("data_collection_audio")
...
def load_data_collection_audio_page():
    ...
    make_label(frame, f"Step 2: Audio Collection ({selected_label.capitalize()})", font=FONT_TITLE).grid(row=0, pady=20)
    ...
    def start_recording():
        ...
        def run_thread():
            ...
            try:
                ...
                # Audio Stream
                p = pyaudio.PyAudio()
                # Assuming Default Input Device. If specific I2S index needed, set input_device_index=X
...
def load_data_collection_page_3():
    ...
    make_label(frame, "Audio will be saved.", font=FONT_NORMAL).pack(pady=10)
    ...
    def confirm_save():
        ...
        try:
            ...
            # NOTE: Image collection is finished. Only saving audio now.
            # 1. Image processing commented out
            ...
            if selected_audio_file and os.path.exists(selected_audio_file):
                ...
                print(f"[INFO] Saved Audio: {final_audio_name}")
...
def load_data_collection_page_4():
    ...
    make_button(frame, "YES (New Capture)", lambda: switch_page("data_collection2"), width=250).pack(pady=10)
...
# Detection Page 1 routes to Camera Page Logic (reused for now, but in real app keep separate if distinct features needed)
def load_data_detection_page_1():
    # We can reuse the same visual style as collection page 1 but redirect next step to detection flow
    # For simplicity in this integration, I'm just reusing the function structure but
    # pointing the "Next" action to detection page 2.
    
    # NOTE: Since the camera page logic is slightly different (capture vs collection), 
    # it is cleaner to copy the 'load_data_collection_page_1' code here and change the destination
    # to 'data_detection2'.
...
def load_data_detection_page_2():
    ...
    # Continue to original detection flow
...
def load_data_detection_page_3():
    ...
    def on_detection_done(processed_path, result):
        ...
        if result is not None:
            switch_page("data_detection5") # Skipped pg 4 (audio capture for detection) for now, directly to results
        else:
